...
class data_manager(object):
    def __init__(self,folder_name):
        ...
        self.train_data , self.train_labels = self.load_set(folder_name)
    ...
    def load_set(self,folder_name):
        ...
        x_rescale = self.rescale(x)
        # data = [ {'features':x_rescale[i,:] , 'label':y[i,:]} for i in range(x_rescale.shape[0]) ]
    ...
    def split_data_into_train_cal(self,data):
        ...
        split_pos = int(len(data)*2.0/3.0)
